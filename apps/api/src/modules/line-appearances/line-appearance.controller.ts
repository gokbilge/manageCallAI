...
const service = new LineAppearanceService(new LineAppearanceRepository(db));
...
function replyNotFound(err: unknown, reply: FastifyReply): void {
  if (err instanceof LineAppearanceNotFoundError || err instanceof AppearanceAssignmentNotFoundError) {
