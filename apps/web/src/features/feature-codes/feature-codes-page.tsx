...
export function FeatureCodesPage() {
  ...
  useEffect(() => {
    if (!featureCodesQuery.data) return;
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: FeatureCodeForm) => {
      if (!selectedFeatureCode) throw new Error('No feature code selected');
  ...
  const selectedActionOption = ACTION_TYPE_OPTIONS.find(
    (option) => option.value === form.watch('action_type'),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.35fr]">
        ...
        <DataCard
          title={isCreating ? 'Create Draft Feature Code' : selectedFeatureCode ? 'Feature Code Details' : 'Feature Code Details'}
        ...
        >
          <div className="space-y-4">
            ...
            {surfaceError ? (
              <WarningState
                title="Conflict or lifecycle check"
            ...
            {!isCreating && !selectedFeatureCode ? (
              <EmptyState
                ...
                description="Select a feature code from the inventory or start a new draft."
              />
            ) : (
              <form className="space-y-4" onSubmit={form.handleSubmit(submitForm)}>
                ...
                <Field label="Action Config JSON">
                  <textarea
                    ...
                    disabled={editorReadOnly}
                    rows={7}
                    {...form.register('action_config_json')}
                  />
                </Field>
